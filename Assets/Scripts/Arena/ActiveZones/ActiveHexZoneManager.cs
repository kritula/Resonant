    ...
    public class ActiveHexZoneManager : MonoBehaviour
    {
        ...
        public void OnZoneCompleted(ActiveHexZoneView zoneView)
        {
            ...
            float? respawnDelayOverride =
                zoneView.ConsumeRespawnDelayOverride();

            _activeZones.Remove(zoneView);
            zoneView.Deactivate();

            if (completedData != null)
                ScheduleRespawn(completedData, respawnDelayOverride);
        }
            ...
            float? delayOverride = null)
        {
            ...
            float delay =
                delayOverride.HasValue
                    ? Mathf.Max(0f, delayOverride.Value)
                    : Random.Range(data.MinRespawnDelay, data.MaxRespawnDelay);
