...
namespace OmniumLessons
{
    public class CharacterSpawnController
    {
        ...
        private void SpawnEnemy(CharacterType enemyType)
        {
            ...
            Vector3 spawnPoint = GetSpawnPointOutsideCamera(enemyType);
        ...
        private Vector3 GetSpawnPointOutsideCamera(CharacterType enemyType)
        {
            Camera camera = Camera.main;
            float spawnY = CharacterSpawnHeights.Get(enemyType);

            if (camera == null)
            {
                Debug.LogWarning("CharacterSpawnController: Camera.main not found.");
                return CharacterSpawnHeights.Apply(
                    CharacterFactory.Player.transform.position + new Vector3(20f, 0f, 20f),
                    enemyType);
            }
            ...
            if (!TryGetCameraGroundBounds(camera, out float minX, out float maxX, out float minZ, out float maxZ))
            {
                ...
                return CharacterSpawnHeights.Apply(
                    CharacterFactory.Player.transform.position,
                    enemyType);
            }
